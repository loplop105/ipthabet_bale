...
from database import (
    ...
    check_rate_limit, backup_database
)
...
from handlers.menu import (
    ...
    after_approval_keyboard, back_button_keyboard
)
    ...
    if state == "AWAITING_CUSTOM_VOLUME":
        if text and text.isdigit():
            ...
            if has_pending_order(user_id):
                BaleAPI.send_message(chat_id, "⚠️ شما یک سفارش در انتظار دارید. لطفاً ابتدا آن را تعیین تکلیف کنید.")
                return
...
def handle_callback_query(cb: dict):
    ...
    # ─── خرید ───
        ...
        if vol_type == "custom":
            ...
            # جلوگیری از سفارش تکراری
            if has_pending_order(user_id):
                BaleAPI.edit_message_text(chat_id, message_id, "⚠️ شما یک سفارش در انتظار دارید.", back_button_keyboard())
                return
    ...
    # ─── تایید ادمین ───
        ...
        order_id = int(data.split("_")[2])
        handle_admin_approve(chat_id, message_id, order_id, user_id)

    # ─── رد ادمین ───
    elif data.startswith("admin_reject_"):
        order_id = int(data.split("_")[2])
        handle_admin_reject(chat_id, message_id, order_id, user_id)
