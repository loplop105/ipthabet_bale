...
def handle_admin_approve(chat_id: int, message_id: int, order_id: int, cb_user_id: int):
    """تایید سفارش - فقط ادمین"""
    # ─── احراز هویت ───
    if not is_admin(cb_user_id):
        BaleAPI.answer_callback_query("", "⛔ دسترسی غیرمجاز")
...
def handle_admin_reject(chat_id: int, message_id: int, order_id: int, cb_user_id: int):
    """رد سفارش - فقط ادمین"""
    if not is_admin(cb_user_id):
