...
def pending_order_keyboard(order_id: int):
    return {"inline_keyboard": [
        [{"text": "💳 تکمیل سفارش معلق", "callback_data": f"pay_pending_{order_id}"}],
        [{"text": "📋 مشاهده جزئیات سفارش", "callback_data": f"view_pending_{order_id}"}],
        [{"text": "❌ لغو و ثبت سفارش جدید", "callback_data": f"cancel_pending_{order_id}"}],
        [{"text": "🏠 منوی اصلی", "callback_data": "menu_main"}],
    ]}
