...
def handle_receipt(user_id: int, chat_id: int, msg: dict, data: dict):
    ...
    # تشخیص نوع سفارش: خرید جدید / تمدید / افزایش حجم
    order_type_val = dict(order).get('order_type', 'new')
    if order_type_val == 'add_volume':
        order_type = "📈 افزایش حجم"
        header = "سفارش افزایش حجم نیاز به تایید دارد"
    elif order['renewal_of'] is not None and order['renewal_of'] > 0:
        order_type = "🔄 تمدید اشتراک"
        header = "سفارش تمدید نیاز به تایید دارد"
    else:
        order_type = "🛒 خرید جدید"
        header = "سفارش جدید نیاز به تایید دارد"
