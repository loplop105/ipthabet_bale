...
class BaleAPI:
    ...
    def send_message(cls, chat_id, text, reply_markup=None, parse_mode="HTML"):
        ...
        try:
            return cls._get_session().post(url, data=data, timeout=Config.API_TIMEOUT).json()
    ...
    @classmethod
    def answer_callback_query(cls, callback_query_id, text="", show_alert=False):
        url = f"{Config.BASE_URL}/answerCallbackQuery"
        data = {"callback_query_id": callback_query_id, "text": text, "show_alert": show_alert}
        try:
            return cls._get_session().post(url, data=data, timeout=(5, 10)).json()
        except Exception as e:
            logger.exception(f"answer_callback_query failed: {e}")
            return None

    @classmethod
    def send_document(cls, chat_id, document, caption=None, reply_markup=None, parse_mode="HTML"):
        """ارسال فایل (PDF, CSV, ZIP و...)"""
        ...
        try:
            ...
            logger.exception(f"send_document failed: {e}")
            return None
