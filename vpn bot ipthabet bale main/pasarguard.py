...
def update_user_expiry(username: str, days: int) -> bool:
    """تمدید زمان انقضای کاربر در پنل - به expire فعلی اضافه می‌کند (حجم دست‌نخورده می‌ماند)"""
    session = _get_session()
    url = f"{Config.PASARGUARD_URL}/api/user/{username}"
    now = int(time.time())
    ...
    if info and isinstance(info, dict):
        ...
        # اگر سرویس هنوز فعال است → به expire فعلی اضافه می‌کنیم
        if current_expire > now:
            new_expire = current_expire + (days * 86400)
            logger.info(f"Extending expiry from {current_expire} + {days}d = {new_expire}")

    # ⚠️ نکته مهم: در تمدید فقط فیلد `expire` را تغییر می‌دهیم.
    # ارسال `data_limit` با مقدار 0 در پنل یعنی «حجم نامحدود (بی‌نهایت)»
    # پس هرگز data_limit را در این تابع بازنویسی نمی‌کنیم تا حجم کاربر از بین نرود.

    # ─── روش اول: PATCH با فقط expire (امن‌ترین - حجم را لمس نمی‌کند) ───
    try:
    ...
    # ─── روش دوم: PUT با فقط expire ───
    try:
        res = session.put(url, json={"expire": new_expire}, timeout=Config.API_TIMEOUT)
        if res.status_code in [200, 201]:
    ...
    # ─── روش سوم (فقط در صورت نیاز): PUT با حفظ حجم واقعی ───
    # در این روش هم اگر حجم واقعی یافت نشد، آن را با 0 بازنویسی نمی‌کنیم.
    if info and isinstance(info, dict):
        ...
        # حفظ صحیح حجم: فقط وقتی data_limit را می‌فرستیم که مقدار واقعی (غیرصفر) داشته باشیم
        current_limit = info.get("data_limit") or info.get("data_limit_bytes") or 0
        if current_limit:
