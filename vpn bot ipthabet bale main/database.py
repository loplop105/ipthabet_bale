...
def init_db():
    with db_transaction() as conn:
        ...
        cursor.execute('''CREATE TABLE IF NOT EXISTS free_renewals (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
...
def has_free_renewal(subscription_id: int) -> bool:
    """بررسی اینکه اشتراک قبلاً تمدید رایگان خود را استفاده کرده است یا نه"""
    with get_db() as conn:
        cursor = conn.cursor()
        cursor.execute('SELECT id FROM free_renewals WHERE subscription_id = ?', (subscription_id,))
        return cursor.fetchone() is not None


def record_free_renewal(subscription_id: int, user_id: int, pg_username: str, extra_days: int) -> bool:
    """ثبت تمدید رایگان برای اشتراک - هر اشتراک فقط یک‌بار (UNIQUE روی subscription_id)"""
    try:
        with db_transaction() as conn:
            conn.execute(
                'INSERT OR IGNORE INTO free_renewals (subscription_id, user_id, pg_username, extra_days) VALUES (?, ?, ?, ?)',
                (subscription_id, user_id, pg_username, extra_days)
            )
        return True
    except Exception as e:
        logger.exception(f"record_free_renewal failed: {e}")
        return False
...
def delete_discount_code(code: str) -> bool:
    ...
    try:
        ...
        logger.exception(f"delete_discount_code failed: {e}")
        return False


def toggle_discount_code(code: str, active: bool) -> bool:
    """فعال/غیرفعال کردن کد تخفیف"""
    try:
        with db_transaction() as conn:
            cursor = conn.cursor()
            cursor.execute(
                'UPDATE discount_codes SET is_active = ? WHERE code = ?',
                (1 if active else 0, code)
            )
            return cursor.rowcount > 0
    except Exception as e:
        logger.exception(f"toggle_discount_code failed: {e}")
        return False
...
def export_users_csv() -> str:
    """ایجاد فایل CSV از کاربران برای خروجی گرفتن"""
    import csv
    import io
    output = io.StringIO()
    writer = csv.writer(output)
    writer.writerow(['user_id', 'username', 'referral_count', 'has_claimed_gift', 'created_at'])

    with get_db() as conn:
        cursor = conn.cursor()
        cursor.execute('SELECT user_id, username, referral_count, has_claimed_gift, created_at FROM users ORDER BY created_at DESC')
        for row in cursor.fetchall():
            writer.writerow([row['user_id'], row['username'], row['referral_count'], row['has_claimed_gift'], row['created_at']])
