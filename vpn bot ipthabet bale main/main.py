            ...
            if has_pending_order(user_id):
                ...
                if pending:
                    ...
                    pending_text = (
                        ...
                        "آیا مایل به لغو سفارش قبلی و ثبت سفارش جدید هستید؟"
                    )
...
def handle_callback_query(cb: dict):
    ...
    # ─── مشاهده سفارش معلق ───
        ...
        if order and order['status'] == 'pending':
            cat_name = Config.CATEGORY_NAMES.get(order['category'], order['category'])
            pending_text = (
                f"📋 <b>جزئیات سفارش معلق</b>\n\n"
                f"🔢 شماره سفارش: <code>{order_id}</code>\n"
                f"📂 دسته‌بندی: <b>{escape_md(cat_name)}</b>\n"
                f"📊 حجم: <b>{order['volume_gb']} گیگابایت</b>\n"
                f"💰 مبلغ: <b>{order['amount']:,} تومان</b>\n"
                f"📅 تاریخ: <b>{order['created_at'][:10] if order['created_at'] else 'نامشخص'}</b>\n\n"
                "آیا مایل به لغو این سفارش هستید؟"
            )
            BaleAPI.edit_message_text(chat_id, message_id, pending_text, pending_order_keyboard(order_id))
        else:
            BaleAPI.edit_message_text(chat_id, message_id, "❌ سفارش معلق یافت نشد.", back_button_keyboard())

    # ─── لغو سفارش معلق ───
    elif data.startswith("cancel_pending_"):
        order_id = int(data.split("_")[2])
        # بررسی مجدد وجود سفارش معلق
        pending = get_pending_order(user_id)
    ...
    # ─── تکمیل سفارش معلق (رفتن به صفحه پرداخت) ───
