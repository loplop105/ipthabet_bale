...
from database import (
    ...
    clear_user_state, create_order, get_user,
    backup_database, get_pending_order, cancel_order
)
...
from monitors.subscription_monitor import start_subscription_monitor
...
from handlers.admin import (
    handle_addcode, handle_admin_approve, handle_admin_reject,
    handle_admin_stats, handle_admin_pending, handle_admin_panel,
    handle_admin_toggle_bot, handle_admin_discounts, handle_discount_info,
    handle_discount_toggle, handle_discount_delete, handle_discount_create,
    handle_discount_create_input, handle_export_users
)
from handlers.payment import handle_discount_input, handle_receipt
from handlers.profile import (
    handle_profile, handle_renew, handle_renew_confirm,
    handle_add_volume, handle_add_volume_custom,
    handle_add_volume_confirm, handle_add_volume_final
)
...
def _show_pending_order(chat_id: int, message_id: int, user_id: int, edit: bool = True, continue_data: dict = None):
    """نمایش کارت سفارش معلق کاربر؛ اگر continue_data داده شود، داده‌های سفارش جدید را ذخیره می‌کند.
    خروجی: سفارش معلق در صورت وجود، در غیر این صورت None"""
    ...
    pending_text = (
        f"⚠️ <b>شما یک سفارش در انتظار دارید!</b>\n\n"
        f" {cat_name}\n"
        f"📊 {pending['volume_gb']} گیگابایت\n\n"
        "می‌توانید پرداخت آن را تکمیل کنید یا آن را لغو و سفارش جدید ثبت کنید:"
    )

    if continue_data:
        continue_data = dict(continue_data)
        continue_data["pending_id"] = pending['id']
        set_user_state(user_id, "AWAITING_CANCEL_AND_CONTINUE", continue_data)

    keyboard = pending_order_keyboard(pending['id'])
    if edit:
        BaleAPI.edit_message_text(chat_id, message_id, pending_text, keyboard)
    else:
        BaleAPI.send_message(chat_id, pending_text, keyboard)
    return pending


def _handle_admin_approve_async(chat_id: int, message_id: int, order_id: int, cb_user_id: int):
    """اجرای تایید سفارش در Task Queue - تا polling بلاک نشود"""
    handle_admin_approve(chat_id, message_id, order_id, cb_user_id)
...
def handle_text_message(msg: dict):
    ...
    # ─── ساخت کد تخفیف از پنل (حالت انتظار) ───
    if state == "AWAITING_ADMIN_DISCOUNT_CODE":
        clear_user_state(user_id)
        handle_discount_create_input(chat_id, text)
        return
    ...
    # ─── حجم دلخواه (مرحله اول: دریافت عدد) ───
    if state == "AWAITING_CUSTOM_VOLUME":
        if text and text.isdigit():
            ...
            amount = volume * price_per_gb

            # ─── مرحله دوم: نمایش تایید نهایی ───
            from handlers.menu import confirm_custom_volume_keyboard
            confirm_text = (
                f"🧾 <b>تایید سفارش</b>\n\n"
                f"📂 دسته‌بندی: <b>{escape_md(category_display)}</b>\n"
                f"📊 حجم: <b>{volume} گیگابایت</b>\n"
                f"💰 نرخ: <b>{price_per_gb:,} تومان/گیگ</b>\n"
                f"⏱ مدت: <b>{Config.DEFAULT_SUBSCRIPTION_DAYS} روزه</b>\n"
                f"💰 مبلغ: <b>{amount:,} تومان</b>\n\n"
                "آیا تایید می‌کنید؟"
            )
            # ذخیره داده برای مرحله نهایی
            set_user_state(user_id, "AWAITING_CUSTOM_VOLUME_CONFIRM", {
                "category": category, "volume": volume, "amount": amount
            })
            BaleAPI.send_message(chat_id, confirm_text, confirm_custom_volume_keyboard(volume))
        else:
            BaleAPI.send_message(chat_id, "⚠️ لطفاً فقط یک عدد معتبر وارد کنید:")
        return

    # ─── حجم دلخواه (مرحله دوم: تایید نهایی) ───
    if state == "AWAITING_CUSTOM_VOLUME_CONFIRM":
        if text and text.lower() in ['بله', 'تایید', 'yes', 'y', 'ok']:
            category = data.get('category')
            ...
            pending = _show_pending_order(
                chat_id, message_id, user_id, edit=False,
                continue_data={"category": category, "volume": volume, "amount": amount}
            )
            if pending:
                return
            ...
            BaleAPI.send_message(chat_id, "⚠️ لطفاً «تایید» را بنویسید یا از دکمه‌ها استفاده کنید.")
        return

    # ─── افزایش حجم دلخواه (مرحله اول: دریافت عدد) ───
    if state == "AWAITING_ADD_VOLUME":
        if text and text.isdigit():
            volume = int(text)
            ...
            if volume < Config.MIN_VOLUME_GB or volume > Config.MAX_VOLUME_GB:
                ...
                return
            clear_user_state(user_id)
            handle_add_volume_confirm(chat_id, message_id, user_id, subscription_id, volume)
        else:
            BaleAPI.send_message(chat_id, "⚠️ لطفاً فقط یک عدد معتبر وارد کنید:")
        return
...
def handle_callback_query(cb: dict):
        ...
        if user and user['has_claimed_gift'] == 1:
            ...
            def _gift_task():
                sub_url, error, pg_username = create_pasarguard_user(
                    volume_gb=Config.GIFT_VOLUME_GB, days=Config.GIFT_DAYS, prefix="gift"
                )
                if sub_url:
                    ...
                    msg_text = (
                        ...
                        f"📊 حجم: **{Config.GIFT_VOLUME_GB} گیگابایت**\n"
                        f"⏱ مدت: **{Config.GIFT_DAYS} روزه**\n\n"
            ...
            task_queue.submit(_gift_task)

    # ─── خرید ───
    elif data == "menu_buy":
        if _show_pending_order(chat_id, message_id, user_id):
            return
        ...
        text = (
            f"📊 دسته‌بندی: <b>{escape_md(category_display)}</b>\n"
            f"⏱ تمامی پلن‌ها <b>{Config.DEFAULT_SUBSCRIPTION_DAYS} روزه</b> هستند.\n\n"
    ...
    # ─── تایید ادمین ───
        ...
        order_id = int(data.split("_")[2])
        # ═══ ارسال به Task Queue ═══
        task_queue.submit(_handle_admin_approve_async, chat_id, message_id, order_id, user_id)
    ...
    # ─── تمدید اشتراک ───
        ...
        handle_renew(chat_id, message_id, user_id)

    elif data.startswith("renew_"):
        subscription_id = int(data.split("_")[1])
        handle_renew_confirm(chat_id, message_id, user_id, subscription_id)

    # ─── تایید حجم دلخواه خرید جدید (مرحله دوم) ───
    elif data.startswith("confirm_custom_vol_"):
        volume = int(data.split("_")[3])
        state, state_data = get_user_state(user_id)
        if state == "AWAITING_CUSTOM_VOLUME_CONFIRM" and state_data:
            ...
            BaleAPI.edit_message_text(chat_id, message_id, "⚠️ لطفاً دوباره تلاش کنید.", back_button_keyboard("menu_buy"))
    ...
    # ─── پنل مدیریت ───
        ...
        if user_id in Config.ADMIN_CHAT_IDS:
            ...
            BaleAPI.answer_callback_query(cb_id, "⛔️ دسترسی غیرمجاز", show_alert=True)

    elif data == "admin_toggle_bot":
        if user_id in Config.ADMIN_CHAT_IDS:
            handle_admin_toggle_bot(chat_id, message_id)
        else:
            BaleAPI.answer_callback_query(cb_id, "⛔️ دسترسی غیرمجاز", show_alert=True)
        ...
        if user_id in Config.ADMIN_CHAT_IDS:
            handle_admin_discounts(chat_id, message_id)
        else:
            BaleAPI.answer_callback_query(cb_id, "⛔️ دسترسی غیرمجاز", show_alert=True)

    elif data == "admin_export_users":
        if user_id in Config.ADMIN_CHAT_IDS:
            handle_export_users(chat_id, message_id)
        else:
            BaleAPI.answer_callback_query(cb_id, "⛔️ دسترسی غیرمجاز", show_alert=True)

    elif data == "disc_create":
        if user_id in Config.ADMIN_CHAT_IDS:
            handle_discount_create(chat_id, message_id, user_id)
        else:
            BaleAPI.answer_callback_query(cb_id, "⛔️ دسترسی غیرمجاز", show_alert=True)

    elif data.startswith("disc_info_"):
        if user_id in Config.ADMIN_CHAT_IDS:
            code = data.replace("disc_info_", "")
            handle_discount_info(chat_id, message_id, code)
        else:
            BaleAPI.answer_callback_query(cb_id, "⛔️ دسترسی غیرمجاز", show_alert=True)

    elif data.startswith("disc_toggle_on_"):
        if user_id in Config.ADMIN_CHAT_IDS:
            code = data.replace("disc_toggle_on_", "")
            handle_discount_toggle(chat_id, message_id, code, True)
        else:
            BaleAPI.answer_callback_query(cb_id, "⛔️ دسترسی غیرمجاز", show_alert=True)

    elif data.startswith("disc_toggle_off_"):
        if user_id in Config.ADMIN_CHAT_IDS:
            code = data.replace("disc_toggle_off_", "")
            handle_discount_toggle(chat_id, message_id, code, False)
        else:
            BaleAPI.answer_callback_query(cb_id, "⛔️ دسترسی غیرمجاز", show_alert=True)

    elif data.startswith("disc_delete_"):
        if user_id in Config.ADMIN_CHAT_IDS:
            code = data.replace("disc_delete_", "")
            handle_discount_delete(chat_id, message_id, code)
        else:
            BaleAPI.answer_callback_query(cb_id, "⛔️ دسترسی غیرمجاز", show_alert=True)

    # ─── افزایش حجم ───
    elif data == "sub_add_volume":
        handle_add_volume(chat_id, message_id, user_id)

    elif data.startswith("addvol_confirm_"):
        # addvol_confirm_{sub_id}_{volume}
        parts = data.split("_")
        subscription_id = int(parts[2])
        volume = int(parts[3])
        handle_add_volume_final(chat_id, message_id, user_id, subscription_id, volume)

    elif data.startswith("addvol_"):
        # addvol_{sub_id}_{vol_type}
        parts = data.split("_")
        subscription_id = int(parts[1])
        vol_type = parts[2]
        if vol_type == "custom":
            handle_add_volume_custom(chat_id, message_id, user_id, subscription_id)
        else:
            volume = int(vol_type)
            handle_add_volume_confirm(chat_id, message_id, user_id, subscription_id, volume)
...
# ═══════════════════════════════════════════
# اجرا (Long Polling) با Graceful Shutdown
# ═══════════════════════════════════════════

def _signal_handler(signum, frame):
    """مدیریت سیگنال خاموش‌سازی"""
    global _running
